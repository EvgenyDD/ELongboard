...
static volatile systime_t SHDN_timeout_msec;

extern bool turnOffNow;
...
void timeout_init(void) {
	...
	SHDN_timeout_msec = SHDN_TIMEOUT;
...
static THD_FUNCTION(timeout_thread, arg) {
	...
	for(;;)
	{
		...
		if(SHDN_timeout_msec != 0 && chVTTimeElapsedSinceX(last_update_time) > MS2ST(SHDN_timeout_msec))
		{
//			mc_interface_unlock();
			RELAY_TURN_OFF();
			turnOffNow = true;

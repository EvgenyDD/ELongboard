...
static THD_FUNCTION(tx_thread, arg) {
	...
	for(;;) {
		...
		if (nosend_cnt >= ALIVE_INTERVAL) {
			...
			buffer_append_uint16(pl, (uint16_t)((float) mc_interface_get_tachometer_abs_value(false) * 0.000209439), &index);  //10m

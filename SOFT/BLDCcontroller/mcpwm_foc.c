...
void mcpwm_foc_generateSound(float frequency, uint32_t delay)
{
	if(frequency == 0.0)
	{
...
#define CURR_SND	1.5
		...
		uint32_t top = (uint32_t)((float)SYSTEM_CORE_CLOCK / frequency);
		TIMER_UPDATE_SAMP_TOP(top - 2, 5, top);

		// Disable timeout
		systime_t tout = timeout_get_timeout_msec();
		float tout_c = timeout_get_brake_current();
		timeout_configure(delay + 100, 0.0);

		mc_interface_lock();

		float duty = RES_SND * CURR_SND / GET_INPUT_VOLTAGE();
		const uint32_t duty_cnt = (uint32_t)((float) TIM1->ARR * duty);
		const uint32_t samp_time = duty_cnt - MCPWM_FOC_INDUCTANCE_SAMPLE_CNT_OFFSET;

		TIMER_UPDATE_DUTY_CURRENTSAMP(0, 0, 0, samp_time);
		start_pwm_hw();

		TIMER_UPDATE_DUTY(duty_cnt, 0, 0);

...
void terminal_process_string(char *str) {
	...
	else if (strcmp(argv[0], "halls") == 0)
	{
		int modeGraph = 0;
		sscanf(argv[1], "%d", &modeGraph);
		debug_halls = modeGraph ? false : true;
		commands_printf(debug_halls ? "  Debug halls enabled" :
						"  Debug halls disabled");
	}
	// The help command

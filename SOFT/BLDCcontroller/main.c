...
extern float smartSpeedIncrement;
extern uint8_t nrfStatus;
extern uint8_t turnOffNow;

bool debug_halls = false;
...
//			{
//				dbgCnt = 0;
//				commands_printf("Halls: %d - %d - %d", READ_HALL1(), READ_HALL2(), READ_HALL3());
//			}
//		}

		/* Smart Speed algorithm */
		static float speedNow = 0;
		float currRPM = mc_interface_get_rpm();
		UTILS_LP_FAST(speedNow, currRPM, 0.1); //Filter speed

		static uint8_t smallDelay = 0;
		if(++smallDelay >= 5)
		{
			smallDelay = 0;

			if(smartSpeedIncrement != 0.0)
			{
				mc_interface_set_pid_speed(currRPM + smartSpeedIncrement);
			}
		}
...
int main(void) {
	...
	for(;;)
	{
		...
		if(palReadPad(GPIOC, 13) /*|| turnOffNow*/)
		{
			turnOffNow = true;
...
//		mcpwm_foc_generateSound(3000, 200);
//		mcpwm_foc_generateSound(0, 1000);
//		mcpwm_foc_generateSound(3500, 200);
	}
}

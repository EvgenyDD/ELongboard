...
//	chMtxLock(&rf_mutex);
	...
	rf_set_crc_type(/*nrf_conf.crc_type*/NRF_CRC_1B/*NRF_CRC_DISABLED*//*???? $$$*/);

	rf_set_retr_retries(/*nrf_conf.retries*/10);
	rf_set_retr_delay(/*nrf_conf.retry_delay*/NRF_RETR_DELAY_1250US);
	rf_set_power(/*nrf_conf.power*/NRF_POWER_0DBM);
	rf_set_speed(/*nrf_conf.speed*/NRF_SPEED_250K);
	rf_set_address_width(NRF_AW_3); // Always use 3 byte address
	rf_set_frequency(2400 + /*(uint16_t)*//*nrf_conf.channel*/115);
...
int rfhelp_send_data_crc(uint8_t *data, uint16_t len)
{
	...
	return rfhelp_send_data(buffer, len + 2, /*nrf_conf.send_crc_ack*/true);
}
